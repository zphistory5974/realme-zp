...
function StatCard({ icon, label, value, sub, highlight }: {
  ...
  return (
    <div style={{ background:'#0d0d0d', border:`1px solid ${highlight ? '#2a2600' : '#1a1a1a'}`, borderRadius:14, padding:'20px 24px' }}>
      <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:10 }}>
        <div style={{ color: highlight ? Y : '#444' }}>{icon}</div>
        <span style={{ fontSize:11, color:'#444', fontWeight:700, letterSpacing:1 }}>{label}</span>
      </div>
      <div style={{ fontSize:36, fontWeight:900, color: highlight ? Y : '#f5f5f0', lineHeight:1 }}>{value}</div>
      ...
      {data.map((d, i) => {
        ...
        return (
          <div key={i} style={{ flex:1, display:'flex', flexDirection:'column', alignItems:'center', gap:4 }}>
            <div style={{ fontSize:10, color: d.count > 0 ? Y : '#333', fontWeight:700 }}>{d.count || ''}</div>
            <div style={{ width:'100%', height:h, borderRadius:3, background: isToday ? Y : '#2a2a2a', minHeight:2, transition:'height 0.3s' }} />
            <div style={{ fontSize:9, color: isToday ? Y : '#333', fontFamily:'monospace' }}>
              {d.date.slice(5)}
            </div>
          </div>
        )
...
export default function AdminPage() {
  ...
  return (
    <div style={{ minHeight:'100vh', background:BK, color:'#f5f5f0', fontFamily:"'Noto Sans KR',sans-serif" }}>

      {/* header */}
      <header style={{ background:BK, borderBottom:'1px solid #1a1a1a', padding:'14px 32px', position:'sticky', top:0, zIndex:10 }}>
        <div style={{ maxWidth:1100, margin:'0 auto', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
          <div style={{ display:'flex', alignItems:'center', gap:12 }}>
            ...
            <div style={{ fontSize:10, color:'#444', border:'1px solid #222', padding:'3px 10px', letterSpacing:2, fontFamily:'monospace' }}>ADMIN</div>
          </div>
          <div style={{ display:'flex', alignItems:'center', gap:12 }}>
            <button onClick={refresh}
              style={{ display:'flex', alignItems:'center', gap:6, fontSize:12, color:'#555', background:'none', cursor:'pointer', padding:'6px 12px', borderRadius:8, border:'1px solid #222' }}
              onMouseEnter={e => e.currentTarget.style.borderColor='#444'}
              onMouseLeave={e => e.currentTarget.style.borderColor='#222'}>
              <RefreshCw style={{ width:13, height:13 }} /> 새로고침
            </button>
            <a href="/" style={{ fontSize:12, color:'#555', textDecoration:'none' }}>← 메인</a>
          </div>
        </div>
      </header>
      ...
      <div style={{ maxWidth:1100, margin:'0 auto', padding:'32px 32px' }}>
        ...
        {tab === 'overview' && (
          <div style={{ display:'flex', flexDirection:'column', gap:24 }}>
            ...
            <div style={{ background:'#0d0d0d', border:'1px solid #1a1a1a', borderRadius:14, padding:'24px 28px' }}>
              <div style={{ fontSize:12, color:'#555', fontWeight:700, marginBottom:20 }}>
                최근 7일 답변 추이
              </div>
              ...
              <div style={{ background:'rgba(255,100,0,0.04)', border:'1px solid #2a1500', borderRadius:14, padding:'20px 24px' }}>
                ...
                <div style={{ fontSize:12, color:'#885533', lineHeight:1.8 }}>
                  {Array.from(fastProfiles).map(hash => {
                    ...
                    return prof ? (
                      <div key={hash}>• <span style={{ color:'#f5f5f0', fontWeight:700 }}>{prof.nickname}</span> — 1분 이내 5개 이상 답변 수신</div>
                    ) : null
                  })}
                </div>
              </div>
        ...
        {tab === 'users' && (
          <div>
            <div style={{ fontSize:13, color:'#555', marginBottom:16 }}>총 {profiles.length}명</div>
              ...
              {profiles.map(prof => {
                ...
                return (
                  <div key={prof.id} style={{
                    background:'#0d0d0d', border:`1px solid ${suspicious ? '#2a1500' : '#1a1a1a'}`,
                    borderRadius:12, padding:'16px 20px',
                    display:'flex', alignItems:'center', justifyContent:'space-between', gap:16,
                  }}>
                    <div style={{ flex:1, minWidth:0 }}>
                      <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:4 }}>
                        <span style={{ fontWeight:900, color:'#f5f5f0', fontSize:14 }}>{prof.nickname}</span>
                        {suspicious && (
                          <span style={{ fontSize:9, color:'#ff6600', border:'1px solid #ff6600', padding:'1px 6px', borderRadius:4 }}>이상</span>
                        )}
                      </div>
                      <div style={{ fontSize:11, color:'#444', fontFamily:'monospace', marginBottom:2 }}>{prof.email}</div>
                      <div style={{ display:'flex', gap:16, fontSize:11, color:'#444' }}>
                        <span>가입: {formatDate(prof.created_at)}</span>
                        <span style={{ color: count > 0 ? Y : '#333' }}>답변 {count}개 수신</span>
                        <span style={{ fontFamily:'monospace', color:'#2a2a2a' }}>{prof.hash}</span>
                      </div>
                    </div>
                    <button onClick={() => deleteProfile(prof.id, prof.hash, prof.nickname)}
                      ...
                      style={{
                        ...
                        borderRadius:8, border:'1px solid #2a1a1a', background:'none',
                        color:'#555', cursor:'pointer', fontSize:12, fontWeight:700,
                      ...
                      onMouseLeave={e => { e.currentTarget.style.borderColor='#2a1a1a'; e.currentTarget.style.color='#555' }}>
        ...
        {tab === 'answers' && (
          <div>
            <div style={{ fontSize:13, color:'#555', marginBottom:16 }}>최근 답변 (총 {answers.length}개)</div>
                  ...
                  <div key={ans.id} style={{
                    ...
                    <div>
                      <div style={{ display:'flex', gap:12, alignItems:'center', marginBottom:3 }}>
                        <span style={{ fontSize:13, fontWeight:700, color:'#f5f5f0' }}>
                      ...
                      <div style={{ fontSize:11, color:'#444' }}>{formatDate(ans.created_at)}</div>
                    </div>
                    <button onClick={() => deleteAnswer(ans.id)}
                      ...
                      style={{
                        ...
                        borderRadius:8, border:'1px solid #2a1a1a', background:'none',
                        color:'#555', cursor:'pointer', fontSize:11, fontWeight:700,
